...
| Feature | inkx | Ink | BubbleTea | Textual | Notcurses | FTXUI | blessed |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Layout engine | ⚡ Flexbox (Flexx 7KB pure JS or Yoga) [^6] | Flexbox (Yoga NAPI, native C++) | Manual | ⚡ CSS subset (grid + flexbox) | Manual ncplane stacking | Flexbox-like (C++) | Manual |
...
| Text input components | ✅ TextInput, ReadlineInput, TextArea (multi-line) | 🔧 `ink-text-input` | 🔧 `textinput` Bubble | ✅ `Input`, `TextArea` | ❌ | ✅ `Input` | ✅ `Textbox`, `Textarea` |
...
| Theming / CSS | 🔶 Style props | 🔶 Style props | ❌ | ⚡ CSS files + live reload | ❌ | 🔶 | 🔶 |
...
- **Per-node dirty tracking**: Interactive updates (keystroke, scroll) take ~169 us for 1000 nodes vs Ink's 20.7 ms full re-render -- 122x faster for the updates that matter.
