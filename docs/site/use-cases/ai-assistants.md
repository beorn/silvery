---
title: Building AI Assistants & Chat Apps with inkx
...
**Scrollable containers for variable-length output.** LLM responses range from one line to hundreds. With `overflow="scroll"` on a Box, inkx measures all children, determines which are visible, and renders only those -- no manual height estimation or virtualization config. Just render your messages and let inkx handle the rest.
