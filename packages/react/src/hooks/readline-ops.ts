...
export function handleReadlineKey(
  ...
  if (key.ctrl && input === "w") {
    ...
    return { value: value.slice(0, newCursor) + value.slice(cursor), cursor: newCursor, yankState: null }
  }
  ...
  if (key.ctrl && input === "h") {
    if (cursor > 0) {
      return { value: value.slice(0, cursor - 1) + value.slice(cursor), cursor: cursor - 1, yankState: null }
    }
