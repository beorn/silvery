...
export type SliceWithInit<S, H extends Record<string, (s: S, ...args: any[]) => any>> = Slice<
  S,
  H
> & {
...
function makeSlice<S, H extends Record<string, (s: S, ...args: any[]) => any>>(
  handlers: H,
): Slice<S, H> {
...
export function createSlice(...args: any[]): any {
  if (args.length === 0) {
    // Curried form
    return <H extends Record<string, (s: any, ...args: any[]) => any>>(handlers: H) =>
      makeSlice(handlers);
  }
  // State factory form
