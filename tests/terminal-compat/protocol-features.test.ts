...
import { ANSI, KittyFlags, enableKittyKeyboard, disableKittyKeyboard, queryKittyKeyboard } from "../../src/output.js"
...
import { copyToClipboard, requestClipboard, parseClipboardResponse } from "../../src/clipboard.js"
import {
  setScrollRegion,
  resetScrollRegion,
  scrollUp,
...
describe("Kitty keyboard protocol sequences", () => {
  ...
  test("enableKittyKeyboard sends CSI > flags u", () => {
    ...
    expect(enableKittyKeyboard(KittyFlags.DISAMBIGUATE | KittyFlags.REPORT_EVENTS)).toBe(`${CSI}>3u`)
...
describe("DECSTBM scroll region sequences", () => {
  ...
  describe("supportsScrollRegions", () => {
    ...
    test("returns true for xterm-compatible TERM values", () => {
      ...
      for (const term of ["xterm-256color", "screen-256color", "tmux-256color", "xterm-kitty"]) {
