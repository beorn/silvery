...
function generateMarkdown(): string {
	...
	const nameWidth = Math.max(
		"Terminal".length,
		...results.map((r) => r.profile.name.length),
	);
	const colWidths = capColumns.map((col) =>
		Math.max(
			col.label.length,
			...results.map((r) => formatValue(col.key, r.caps[col.key]).length),
		),
	);
	...
	for (const { profile, caps } of results) {
		const values = capColumns.map((col, i) =>
			padRight(formatValue(col.key, caps[col.key]), colWidths[i]!),
		);
	...
	lines.push("| --- | --- |");
	lines.push(
		"| Colors | Color support: 24-bit (truecolor), 256, 16 (basic), or - (none) |",
	);
	lines.push(
	...
	lines.push(
		"| Sixel | [Sixel graphics](https://en.wikipedia.org/wiki/Sixel) (inline images) |",
	);
	lines.push(
	...
	lines.push(
		"| Hyperlinks | [OSC 8 hyperlinks](https://gist.github.com/egmontkob/eb114294efbcd5adb1944c9f3cb5feda) |",
	);
	lines.push(
		"| Notify | Terminal notifications (OSC 9 for iTerm2, OSC 99 for Kitty) |",
	);
	lines.push(
		"| Paste | [Bracketed paste mode](https://cirw.in/blog/bracketed-paste) (DEC 2004) |",
	);
	...
	lines.push("");
	lines.push(
		"Capabilities are detected synchronously from environment variables:",
	);
	lines.push("");
	lines.push(
		"- `TERM` -- terminal type identifier (e.g., `xterm-256color`, `xterm-kitty`)",
	);
	lines.push(
	...
	lines.push("");
	lines.push(
		"No I/O is performed (no terminal queries). This means detection is instant",
	);
	lines.push(
		"but limited to what env vars reveal. Some capabilities (like Kitty keyboard)",
	);
