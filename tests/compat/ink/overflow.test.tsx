/**
 * Ink compat test: overflow clipping (from ink/test/overflow.tsx)
 *
 * Tests overflowX, overflowY, and combined overflow="hidden" containers.
 * Uses per-axis clip bounds in content-phase.ts computeChildClipBounds.
 */
...
test.todo("overflowX - multiple text nodes in a box with border inside overflow container — text concat rendering difference", () => {
  const output = renderToString(
...
test.todo("overflowX - box intersecting with left edge of overflow container with border — left clip off-by-one", () => {
  const output = renderToString(
    <Box width={8} overflowX="hidden" borderStyle="round">
      <Box marginLeft={-3} width={12} flexShrink={0}>
        <Text>Hello World</Text>
