...
describe('HorizontalVirtualList', () => {
	...
	it('scrolls to selected item', () => {
		const items = Array.from({ length: 100 }, (_, i) => ({ id: `item-${i}`, name: `I${i}` }));
	...
	it('shows overflow indicators when enabled', () => {
		const items = Array.from({ length: 50 }, (_, i) => ({ id: `item-${i}`, name: `I${i}` }));
	...
	it('respects maxRendered limit', () => {
		const items = Array.from({ length: 100 }, (_, i) => ({ id: `item-${i}`, name: `N${i}` }));
	...
	it('edge-based scrolling preserves offset when not selected', () => {
		const items = Array.from({ length: 20 }, (_, i) => ({ id: `item-${i}`, name: `I${i}` }));
