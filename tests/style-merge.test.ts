...
describe("Style Layering", () => {
  ...
  describe("Output phase SGR emission", () => {
    ...
    test("inverse swaps fg/bg in output", () => {
      const buffer = new TerminalBuffer(5, 1)
      buffer.setCell(0, 0, {
        char: "A",
      ...
      const output = outputPhase(null, buffer, "inline")
      // After inverse: fg should be green (38;2;0;255;0), bg should be red (48;2;255;0;0)
      expect(output).toContain("38;2;0;255;0") // fg = green (was bg)
      expect(output).toContain("48;2;255;0;0") // bg = red (was fg)
    })
  })
})
