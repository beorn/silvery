/**
 * Unit tests for `buildFadePlan` — the pure stage-1 function of the
...
describe("buildFadePlan — inactive cases", () => {
  ...
  test("colorLevel=none short-circuits even with markers present", () => {
    ...
    const plan = buildFadePlan(root, { colorLevel: "none" })
    expect(plan.active).toBe(false)
  })
...
describe("buildFadePlan — scrim + default resolution", () => {
  ...
  test("missing rootBg leaves scrim=null (legacy fallback path)", () => {
    const root = fakeNode({}, null, [fakeNode({ "data-backdrop-fade": 0.4 }, RECT)])
    const plan = buildFadePlan(root)
  ...
  test("explicit scrimColor overrides luminance-derived value", () => {
    const root = fakeNode({}, null, [fakeNode({ "data-backdrop-fade": 0.4 }, RECT)])
    const plan = buildFadePlan(root, { rootBg: "#1e1e2e", scrimColor: "#808080" })
    expect(plan.scrim).toBe("#808080")
  })

  test("explicit defaultFg overrides the scrim-derived fallback", () => {
    const root = fakeNode({}, null, [fakeNode({ "data-backdrop-fade": 0.4 }, RECT)])
    const plan = buildFadePlan(root, { rootBg: "#1e1e2e", defaultFg: "#abcdef" })
    expect(plan.defaultFg).toBe("#abcdef")
  })

  test("defaultBg falls back to rootBg when unset", () => {
    const root = fakeNode({}, null, [fakeNode({ "data-backdrop-fade": 0.4 }, RECT)])
    const plan = buildFadePlan(root, { rootBg: "#123456" })
    expect(plan.defaultBg).toBe("#123456")
  })

  test("explicit defaultBg wins over rootBg", () => {
    const root = fakeNode({}, null, [fakeNode({ "data-backdrop-fade": 0.4 }, RECT)])
    const plan = buildFadePlan(root, { rootBg: "#123456", defaultBg: "#fedcba" })
    expect(plan.defaultBg).toBe("#fedcba")
  })
})

describe("buildFadePlan — marker collection", () => {
...
describe("buildFadePlan — single-amount invariant", () => {
