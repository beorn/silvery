...
describe("ListView scroll-contract fuzz sweep (cols × rows × heights × scrollTo)", () => {
  test(
    "property: all 4 invariants hold across random (cols, rows, count, height-shape, scrollTo)",
    () => {
      fc.assert(
        fc.property(
          ...
          (cols, rows, count, shape, heightSeed, scrollSeed) => {
            ...
            const scrollTo =
              scrollSeed < 0.25 ? undefined : Math.floor(scrollSeed * count) % count
  ...
  test(
    "property: viewport exact-fill — no indicator, no gap",
    () => {
      fc.assert(
        fc.property(
          fc.constantFrom(...COL_CHOICES),
  ...
  test(
    "property: viewport partial-fill — content < viewport, no indicator",
    () => {
      fc.assert(
        fc.property(
          fc.constantFrom(...COL_CHOICES),
