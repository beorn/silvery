...
import { Box, Text, initYogaEngine, setLayoutEngine } from '../src';
import { renderString, renderStringSync } from '../src/render-string.js';
...
describe('renderString', () => {
	...
	test('renders plain text with plain option', async () => {
		const output = await renderString(<Text color="green">Green</Text>, { plain: true });
...
describe('renderStringSync', () => {
	...
	test('works identically to async version when engine is ready', async () => {
		...
		const asyncOutput = await renderString(<Text color="blue">Test</Text>, { plain: true });
		const syncOutput = renderStringSync(<Text color="blue">Test</Text>, { plain: true });
