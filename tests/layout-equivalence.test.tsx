...
function renderWithBothEngines(
	...
	// Render with Yoga
	const yogaRender = createTestRenderer({ layoutEngine: yogaEngine, columns, rows });
	const yogaResult = yogaRender(element);
	const yogaFrame = yogaResult.lastFrame();

	// Render with Flexx
	const flexxRender = createTestRenderer({ layoutEngine: flexxEngine, columns, rows });
...
function expectEquivalent(
	...
	const { yogaNormalized, flexxNormalized, yogaFrame, flexxFrame } = renderWithBothEngines(
		element,
		options,
	);
