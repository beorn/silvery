...
import { render, Box, Text, useInput, useApp, type Key } from '../../src/index.js';
	...
	return (
		<Box flexDirection="column">
			<Box>
				...
				{hasSubtasks && (
					<Text dim> ({task.subtasks!.length} subtasks)</Text>
				)}
			</Box>
	...
	return (
		...
		>
			<Text>
				...
				<Text dim>/{total} completed ({percent}%)</Text>
			</Text>
			<Text dim>
				Item {cursor + 1}/{total} | Scroll: {scrollOffset + 1}-{Math.min(scrollOffset + visibleCount, total)}
			</Text>
		</Box>
	);
}
...
function TaskList(): JSX.Element {
	...
	return (
		<Box flexDirection="column" padding={1}>
			<Box marginBottom={1}>
				<Text bold color="yellow">Task List</Text>
				<Text dim>
					{' '}| j/k or arrows to navigate | space to toggle | enter to expand | q to quit
				</Text>
			</Box>
