 ...
 *
 * Run: bun examples/apps/terminal.tsx
 */
      ...
      <Box flexDirection="column" flexGrow={1}>
        ...
        {events.length === 0 ? (
          ...
          <Box flexDirection="column" overflow="scroll" scrollTo={events.length - 1}>
            {events.map((e, i) => (
              <Text key={e.index} color={i < events.length - 1 ? "$fg-muted" : undefined}>
...
function ModBadge({ mod, active }: { mod: ModDef; active: boolean }) {
  if (active) {
    return (
      <Text backgroundColor={mod.color} color="$bg-inverse" bold>
      ...
      {history.length > 0 && (
        <Box flexDirection="column">
          ...
          <Box flexDirection="column" overflow="scroll" scrollTo={history.length - 1}>
            {history.map((h, i) => (
              <Text key={i} color={i < history.length - 1 ? "$fg-muted" : undefined}>
                <Small>{h.time}</Small>{" "}
                <Text
                  color={
                    h.action === "copy"
