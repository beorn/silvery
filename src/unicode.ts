...
/**
 * Quick check for Private Use Area characters.
 * BMP PUA: U+E000-U+F8FF (used by Nerd Fonts for terminal icons).
 * Terminals render these at 2-cell width when the font provides a glyph.
 */
...
export function displayWidth(text: string): number {
  ...
  // use string-width directly. PUA chars (U+E000-U+F8FF, Nerd Font icons) need
  // the slow path because graphemeWidth overrides their width to 2.
 ...
 *
 * Also overrides for Private Use Area (PUA) characters (U+E000-U+F8FF),
 * which are used by Nerd Fonts for icons. Modern terminals (Ghostty, Kitty,
 * iTerm2) render PUA glyphs at 2-cell width when followed by whitespace.
 * Since icons are typically followed by a space separator, treating them as
 * 2-wide prevents the last character of adjacent text from being clipped
 * at the column boundary. See km-tui.col-header-trunc.
 *
...
export function graphemeWidth(grapheme: string): number {
  ...
  // Private Use Area characters (Nerd Font icons): terminals render these
  // at 2-cell width when the font provides a glyph. string-width reports 1
  // because PUA has no standard width, but all major terminal emulators
  // (Ghostty, Kitty, iTerm2, WezTerm) render nerd font PUA glyphs wide.
  if (isPrivateUseArea(grapheme)) return 2
  return width
}

/**
 * Check if a grapheme is in the Unicode Private Use Area (PUA).
 * PUA ranges: U+E000-U+F8FF (BMP), U+F0000-U+FFFFD, U+100000-U+10FFFD (supplementary)
 * Nerd Fonts uses U+E000-U+F8FF extensively for terminal icons.
 */
function isPrivateUseArea(grapheme: string): boolean {
  const cp = grapheme.codePointAt(0)
  if (cp === undefined) return false
