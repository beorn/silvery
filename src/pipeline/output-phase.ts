...
export function setOutputCaps(
  _caps: Partial<
    Pick<TerminalCaps, "underlineStyles" | "underlineColor" | "colorLevel">
  >,
): void {
...
export type OutputCaps = Pick<
  TerminalCaps,
  "underlineStyles" | "underlineColor" | "colorLevel"
>;
...
function updateInlineCursorRow(
  ...
  if (cursorPos?.visible) {
    ...
    state.prevCursorRow =
      visibleRow >= 0 && visibleRow < maxOutputLines
        ? visibleRow
        : maxOutputLines - 1;
  } else {
...
function styleTransition(
  ...
  if (
    ...
    !colorEquals(oldStyle.underlineColor, newStyle.underlineColor)
  ) {
    if (
      newStyle.underlineColor === null ||
      newStyle.underlineColor === undefined
    ) {
...
export function outputPhase(
  ...
  if (!prev) {
    ...
    if (
      mode === "inline" &&
      inlineState.prevBuffer &&
      inlineState.prevCursorRow >= 0
    ) {
    ...
    const firstOutput = bufferToAnsi(
      next,
      mode,
      ctx,
      mode === "inline" ? termRows : undefined,
    );
    ...
    if (mode === "inline") {
      ...
      const firstMaxOutput =
        termRows != null
          ? Math.min(firstContentLines, termRows)
          : firstContentLines;
      ...
      if (inlineState.prevCursorRow >= 0) {
        const clearDistance =
          termRows ??
          Math.max(inlineState.prevCursorRow, inlineState.prevOutputLines - 1);
        if (clearDistance > 0) {
      ...
      return (
        prefix +
        firstOutput +
        inlineCursorSuffix(cursorPos ?? null, next, termRows)
      );
    }
...
function inlineCursorSuffix(
  ...
  const maxOutputLines =
    termRows != null
      ? Math.min(lastContentLine + 1, termRows)
      : lastContentLine + 1;
...
function inlineFullRender(
  ...
  const terminalScroll =
    termRows != null ? Math.max(0, rawCursorOffset - (termRows - 1)) : 0;
...
function formatColor(
  c: number | { r: number; g: number; b: number } | null,
): string {
...
function verifyOutputEquivalence(
  ...
  if (process.env.INKX_DEBUG_OUTPUT) {
    // eslint-disable-next-line no-console
    console.error(
      `[VERIFY] freshPrev len=${freshPrev.length} incrOutput len=${incrOutput.length}`,
    );
    // Show incrOutput as escaped string
    const escaped = incrOutput
      .replace(/\x1b/g, "\\e")
      .replace(/\r/g, "\\r")
      .replace(/\n/g, "\\n");
    // eslint-disable-next-line no-console
  ...
  const screenIncr = replayAnsiWithStyles(
    w,
    vtHeight,
    freshPrev + incrOutput,
    ctx,
  );
  ...
  for (let y = 0; y < compareHeight; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (incr.strikethrough !== fresh.strikethrough)
        diffs.push(
          `strikethrough: ${incr.strikethrough} vs ${fresh.strikethrough}`,
        );
...
function verifyAccumulatedOutput(
  ...
  const screenAccumulated = replayAnsiWithStyles(
    w,
    h,
    accState.accumulatedAnsi,
    ctx,
  );
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (accum.strikethrough !== fresh.strikethrough)
        diffs.push(
          `strikethrough: ${accum.strikethrough} vs ${fresh.strikethrough}`,
        );
