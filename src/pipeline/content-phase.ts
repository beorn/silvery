...
function renderScrollContainerChildren(
	...
	const needsViewportClear = hasPrevBuffer && (scrollOffsetChanged || node.subtreeDirty);

	const childHasPrev =
		scrollOffsetChanged || node.childrenDirty || parentRegionCleared || needsViewportClear
			? false
			: hasPrevBuffer;
...
function renderNormalChildren(
	...
	const childHasPrev =
		node.childrenDirty || childPositionChanged || parentRegionCleared ? false : hasPrevBuffer;
