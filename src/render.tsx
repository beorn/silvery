...
function InkxApp({
  ...
  const handleReadable = useCallback(() => {
    ...
    function processInput() {
      ...
      debug(
        "stdin.read() returned: %s",
        chunk === null ? "null" : JSON.stringify(chunk),
      );
  ...
  const setRawMode = useCallback(
    (enabled: boolean) => {
      ...
      if (enabled) {
        ...
        rawModeCountRef.current++;
        debug(
          "setRawMode: rawModeCount incremented to %d",
          rawModeCountRef.current,
        );
      } else {
        rawModeCountRef.current = Math.max(0, rawModeCountRef.current - 1);
        debug(
          "setRawMode: rawModeCount decremented to %d",
          rawModeCountRef.current,
        );
        if (rawModeCountRef.current === 0) {
...
class InkxInstance {
  ...
  render(element: ReactNode): void {
    ...
    debug(
      "InkxInstance.render() updateContainerSync complete in %dms",
      Date.now() - startTime,
    );
...
export async function render(
  ...
  debug(
    "render(): instance.render() complete, total: %dms",
